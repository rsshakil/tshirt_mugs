...
class WC_4JP_Admin_Screen {
	...
	public function __construct() {
		...
		$this->wc4jp_plugin = new Framework\WC4JP_Plugin();
		$this->prefix =  'wc4jp-';
	}
	/**
	 * Admin Menu
	...
	function wc4jp_setting_init(){
		...
		add_settings_field( 'wc4jp_options_no_ja', __( 'No Japanese Address', 'woocommerce-for-japan' ), array( $this, 'wc4jp_options_no_ja' ), 'wc4jp_setting', 'wc4jp_general' );
...
//		add_settings_field( 'wc4jp_delivery_time_zone', __( 'Delivery tracking', 'woocommerce-for-japan' ), array( $this, 'wc4jp_delivery_tracking' ), 'wc4jp_shipment', 'wc4jp_delivery_tracking' );
		...
		if( isset( $_POST['_wpnonce']) and isset($_GET['page']) and $_GET['page'] == 'wc4jp-options' ){
			if((isset($_GET['tab']) and $_GET['tab'] == 'setting') or is_null($_GET['tab'])){
				...
				$add_methods = array('yomigana', 'honorific-suffix', 'company-name', 'free-shipping', 'zip2address', 'yahoo-app-id', 'no-ja');
	...
	public function wc4jp_options_yomigana() {
		...
		$this->wc4jp_plugin->wc4jp_input_checkbox('yomigana', $descritpion, $this->prefix);
	}
	/**
	...
	public function wc4jp_options_honorific_suffix() {
		...
		$descritpion = $this->wc4jp_description_address_pattern( $title );
		$this->wc4jp_plugin->wc4jp_input_checkbox('honorific-suffix', $descritpion, $this->prefix);
	}
	/**
	 * Company Name option.
	 * 
	...
	public function wc4jp_options_company_name() {
		...
		$descritpion = $this->wc4jp_description_address_pattern( $title );
		$this->wc4jp_plugin->wc4jp_input_checkbox('company-name', $descritpion, $this->prefix);
	}
	/**
	...
	public function wc4jp_options_free_shipping() {
		...
		$descritpion = $this->wc4jp_description_address_pattern( $title );
		$this->wc4jp_plugin->wc4jp_input_checkbox('free-shipping', $descritpion, $this->prefix);
	}
	/**
	 * Free Shipping Display option.
	 * 
	...
	public function wc4jp_options_zip2address() {
		...
		$descritpion = $this->wc4jp_plugin->wc4jp_description_check_pattern( $title );
		$this->wc4jp_plugin->wc4jp_input_checkbox('zip2address', $descritpion, $this->prefix);
	}
	/**
	 * Free Shipping Display option.
	 * 
	...
	public function wc4jp_options_yahoo_app_id() {
		...
		$this->wc4jp_plugin->wc4jp_input_text('yahoo-app-id', $descritpion, 60, '',$this->prefix);
	}
	 ...
	 * @return mixed
	 */
	public function wc4jp_options_no_ja() {
		$descritpion = __( 'Eliminate the order of addresses for Japan.', 'woocommerce-for-japan' );
		$this->wc4jp_plugin->wc4jp_input_checkbox('no-ja', $descritpion, $this->prefix);
	}
	/**
	...
	public function wc4jp_delivery_date_designation(){
		...
		$descritpion = $this->wc4jp_plugin->wc4jp_description_check_pattern( $title );
		$this->wc4jp_plugin->wc4jp_input_checkbox('delivery-date', $descritpion, $this->prefix);
	}
	/**
	...
	public function wc4jp_start_date(){
		...
		$this->wc4jp_plugin->wc4jp_input_number('start-date', $descritpion, 2,$this->prefix);
	}
	/**
	...
	public function wc4jp_reception_period(){
		...
		$this->wc4jp_plugin->wc4jp_input_number('reception-period', $descritpion, 7,$this->prefix);
	}
	/**
	...
	public function wc4jp_unspecified_date(){
		...
		$this->wc4jp_plugin->wc4jp_input_text('unspecified-date', $descritpion, 60, __( 'No specified date', 'woocommerce-for-japan' ), $this->prefix);
	}
	/**
	 * Unspecified for delivery date.
	 * 
	...
	public function wc4jp_date_format(){
		...
		$this->wc4jp_plugin->wc4jp_input_text('date-format', $descritpion, 10, 'Y/m/d', $this->prefix);
	}
	/**
	...
	public function wc4jp_delivery_deadline(){
		...
		$this->wc4jp_plugin->wc4jp_input_time('delivery-deadline', $descritpion, '15:00', $this->prefix);
	}
	/**
	...
	public function wc4jp_no_delivery_weekday(){
		...
		foreach($weekdays as $weekday){
			...
			echo $weekday['label'].$this->wc4jp_plugin->wc4jp_input_checkbox($weekday['name'], null, $this->prefix).' ';
			if(isset($weekday['bold']))echo '</strong>';
		}
	...
	public function wc4jp_holiday_term(){
		...
		$this->wc4jp_plugin->wc4jp_input_date_term($start_date, $end_date, $descritpion, $this->prefix);
	}
	...
	public function wc4jp_delivery_time_zone(){
		...
		$descritpion = $this->wc4jp_plugin->wc4jp_description_check_pattern( $title );
		$this->wc4jp_plugin->wc4jp_input_checkbox('delivery-time-zone', $descritpion, $this->prefix);
	}
	/**
	...
	public function wc4jp_unspecified_time(){
		...
		$this->wc4jp_plugin->wc4jp_input_text('unspecified-time', $descritpion, 60, __( 'No designated time zone', 'woocommerce-for-japan' ), $this->prefix);
	}
	/**
	...
	public function wc4jp_delivery_tracking(){
		...
		$descritpion = $this->wc4jp_plugin->wc4jp_description_check_pattern( $title );
		$this->wc4jp_plugin->wc4jp_input_checkbox('delivery-tracking', $descritpion, $this->prefix);
	}
	/**
	...
	public function wc4jp_options_bankjp() {
		...
		$descritpion = $this->wc4jp_plugin->wc4jp_description_payment_pattern( $title );
		$this->wc4jp_plugin->wc4jp_input_checkbox('bankjp', $descritpion, $this->prefix);
	}
	/**
	...
	public function wc4jp_options_postofficebank() {
		...
		$descritpion = $this->wc4jp_plugin->wc4jp_description_payment_pattern( $title );
		$this->wc4jp_plugin->wc4jp_input_checkbox('postofficebank', $descritpion, $this->prefix);
	}
	/**
	 * Pay at store option.
	 * 
	...
	public function wc4jp_options_atstore() {
		...
		$descritpion = $this->wc4jp_plugin->wc4jp_description_payment_pattern( $title );
		$this->wc4jp_plugin->wc4jp_input_checkbox('atstore', $descritpion, $this->prefix);
	}
	/**
	 * Postal transfer option.
	 * 
	...
	public function wc4jp_options_cod2() {
		...
		$descritpion = $this->wc4jp_plugin->wc4jp_description_payment_pattern( $title );
		$this->wc4jp_plugin->wc4jp_input_checkbox('cod2', $descritpion, $this->prefix);
	}
	/**
