design.tools.crop = {
	...
	btn: function(){
		...
		if( div.find('.btn-crop-img').length == 0 )
		{
			...
			jQuery('#options-add_item_clipart .btn-group-custom').append('<li id="filter-link-smp" class="btn-crop-img" data-type="filter" onclick="design.tools.crop.setup();"><i class="fa fa-crop"></i> <small class="clearfix">'+cropActionLabel+'</small></li>');
		}
	},
	...
	wapper: function(){
		...
		jQuery('.mask-items-area').append('<div class="wapper-item-crop"><div class="area-item-crop"></div><span class="bg-item-crop"><div class="bg-crop"></div><div class="dg-options-toolbar" id="crop-toolbar">'+html+'</div></span></div>');
		this.mask();
	},
	mask: function(div){
	...
	cropBtn: function(hidden){
		...
		jQuery('#crop-toolbar').css({'top': top+'px', 'left':left+'px'}).show();
	}
}
