				...
				<?php 
					if(count($print_types))
					{
						foreach($print_types as $key => $print)
						{
							...
							echo '<div class="amodal" id="'.$key.'" data-description="'.strip_tags($print['short_description']).'" title="'.$print['title'].'">';
							echo '<h4><a href="javascript:void(0);">'.$print['title'].'</a></h4>';
							echo '<p class="printing-content">'.(isset($print['short_description']) ? $print['short_description'] : '').'</p>';
							echo '</div><button class="btn btn-default" onclick="changePrintintType(jQuery(\'#'.$key.'\').get(0))">'.lang('quick_designer_change', true).'</button></div>';
						}
					}
				?>
			</div>
