 ...
 class Design extends Controllers{
	...
	public function user_design($segment = 1)
	{
		...
		if($files && count($files) > 0)
		{
			foreach($files as $file) 
			{
				...
				$fdesign = $cache->get($file_name);
				if (isset($fdesign['vectors'])) continue;
				$cache_arr[$file_name] = array_reverse($fdesign); // for sort
			}			
		}
		if (count($cache_arr)) {
			foreach ($cache_arr as &$rows) {
				foreach ($rows as &$row) {
					if (!isset($row['design_id'])) {
						$row['design_id'] = $row['id'];
					}

					if (!isset($row['print'])) {
						if (isset($row['id']))
							$fprint = $cache->get($row['id']);
						else
							$fprint = $cache->get($row['design_id']);
						$row['print'] = $fprint['print'];
					}
				}
			}
		}
		...
		if($perpage == 'all')
		{
			foreach($cache_arr as $keys => &$values)
			{
				...
				{
					$tmp_key = isset($value['id']) ? $value['id'] : (isset($value['desing_id']) ? $value['desing_id'] : (isset($value['key']) ? $value['key'] : ''));
					if ($key != $tmp_key && !empty($tmp_key)) {
						$key = $tmp_key;
					}
	...
	public function admin_design($segment = 1)
	{
		...
		if (count($cache_arr)) {
			foreach ($cache_arr as &$rows) {
				foreach ($rows as &$row) {
					if (!isset($row['design_id']) && isset($row['id'])) {
						$row['design_id'] = $row['id'];
					}
					...
					if (!isset($row['print'])) {
						if (isset($row['id']))
							$fprint = $cache->get($row['id']);
						elseif (isset($row['design_id']))
							$fprint = $cache->get($row['design_id']);
						if(!isset($fprint['print']))
							continue;
						$row['print'] = $fprint['print'];
					}
				}
		...
		if (count($data['admin_design'])) {
			foreach ($data['admin_design'] as &$row) {
				unset($row['vectors']);
				unset($row['vector']);
			}
		}
