...
class submitPrice extends Controllers
{
	...
	public function index()
	{
		...
		//sort array().
		$sort = array();
		if ($submit_prices != false && $submit_prices != null && is_object($submit_prices)) {
	...
	public function page($segment = 0)
	{
		...
		$submit_prices = json_decode($products);	
		if ($submit_prices == false || $submit_prices == null) {
			$submit_prices = array();
		}
		
		if(isset($submit_prices) && count($submit_prices))
		{
			$search = array();
			foreach($submit_prices as $key=>$val)
			{
