...
class P9f_frontend
{
	function __construct()
	{
		...
		/* add thumb from URL to design */
		add_action('P9f_product_design', array($this, 'add_upload_design'), 20);
		add_action('woocommerce_after_shop_loop', array($this, 'add_upload_design'), 20);
	}
	...
	* change url of button design
	...
	function add_upload_design()
	{
		if(isset($_GET['thumb']) && $_GET['thumb'] != '')
		{
			$this->add_shortcode_js();
			add_filter('P9f_product_design_data', array($this, 'design_add_thumb'), 40, 1 );
			$extra = '';
			if(!is_product())
			{
				$extra = 'var is_page_list = 1;';
				$extra .= 'var design_thumb = "'.$_GET['thumb'].'";';
				$extra .= 'var siteURL = "'.network_site_url('tshirtecommerce').'/";';
				$extra .= 'var URL_d_home = "'.site_url().'";';
			}
			echo '<script type="text/javascript">var load_design_from_url = 1;'.$extra.'</script>';
		}
	}

	function add_shortcode_js()
	{
		global $P9f;
	...
	function design_add_thumb($data)
	{
		$data['idea'] = array(
			'color' => 'FFFFFF',
			'thumb' => array(
