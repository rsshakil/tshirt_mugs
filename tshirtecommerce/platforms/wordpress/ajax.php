	...
	* Get URL of product woocommerce
	...
	public function get_variation_id()
	{
		if( isset( $_GET['product_id'] ) )
		{
			...
			if($variation_id == 0)
			{
				$data['image'] 	= wp_get_attachment_image_src( get_post_thumbnail_id( $product_id ), 'single-post-thumbnail' );
				if($data['image'] == '')
				{
					$data['image'] 	= network_site_url('tshirtecommerce/assets/images/photo.png');
				}
			}
		}
	...
	function get_variations()
	{
		if( isset( $_GET['product_id'] ) )
		{
			...
			global $product, $wc_cpdf, $P9f;
			$product 			= wc_get_product($product_id);
			$product_design_id 	= $wc_cpdf->get_value($product_id, '_product_id');
			if($product->is_type( 'variable' ) && $product_design_id)
			{
				$ids 			= explode(':', $product_design_id);
				if(count($ids) > 1)
				{
					echo '';
					exit();
				...
				if(count($variations))
				{
					$woo_image 	= wp_get_attachment_image_src( get_post_thumbnail_id( $product_id ), 'single-post-thumbnail' );
					if($woo_image == false)
					{
						$woo_image 	= network_site_url('tshirtecommerce/assets/images/photo.png');
					}
					$title 			= $product->get_title();

					foreach($variations as $variation)
					{
						$options[$variation['variation_id']] = array(
						...
						if(isset($variation['image']) && isset($variation['image']['src']) && $variation['image']['src'] != '')
						{
							$options[$variation['variation_id']]['image'] = $variation['image']['src'];
						}
						else
						{
							$options[$variation['variation_id']]['image'] = $woo_image;
						}

						$design_id = get_post_meta($variation['variation_id'], '_variable_design_id', true);
						if($design_id)
						{
							$options[$variation['variation_id']]['design_id'] = $design_id;
						}
					}
					echo '<script type="text/html" id="woo-variations-data">'.json_encode($options).'</script>';
				}
			}
		}
		exit;
